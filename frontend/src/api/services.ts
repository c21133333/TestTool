...
import type { ApiCase, AuditLogListResult, AuthSession, Environment, Execution, ExecutionListResult, Project, Report, Suite, User } from './types';
...
export function createApi(token: string | null) {
  ...
  return {
    ...
    listProjects: () => client.request<Project[]>('/projects'),
    ...
    listSuites: (projectId?: number) =>
      client.request<Suite[]>(`/suites${projectId ? `?project_id=${projectId}` : ''}`),
    ...
    listCases: (suiteId?: number) =>
      client.request<ApiCase[]>(`/cases${suiteId ? `?suite_id=${suiteId}` : ''}`),
    ...
    listEnvironments: (projectId?: number) =>
      client.request<Environment[]>(`/environments${projectId ? `?project_id=${projectId}` : ''}`),
    ...
    listUsers: () => client.request<User[]>('/users'),
